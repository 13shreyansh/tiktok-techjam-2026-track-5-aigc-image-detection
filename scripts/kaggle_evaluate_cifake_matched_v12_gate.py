#!/usr/bin/env python3
"""Falsify the v12 candidates on a frozen, same-source CIFAKE gate.

This evaluator never trains, tunes, calibrates or selects a threshold.  It
verifies the complete gate package, proves identity separation from the
mounted v12 manifests, checks the exact checkpoint hash, then evaluates clean
plus the 19 individually applied workshop conditions.  The interpretation
bands were frozen in CIFAKE_MATCHED_SOURCE_V12_GATE_PLAN.json before scoring.
"""
...
SEED = 20260831
GATE_PACKAGE_NAME = "cifake-matched-source-v12-gate.zip"
GATE_PACKAGE_SHA256 = "b91363fef08bceb3c72f86ca4e5d4fce8b0c0a530d79e56b431dfa8a0087d383"
GATE_INVENTORY_SHA256 = "1b62ff5df23538879a4e922a68648fcee73b73d39fc359b6db71804e69c14f5c"
GATE_MANIFEST_SHA256 = "57178fc43a3488851b5f973cce4edeadd95eac4e17154c55e8338e4070f56656"
...
GATE_WORK_ROOT = Path("/kaggle/working/cifake-matched-source-v12-gate")
...
def gate_decision(clean_auc: float, worst_condition_auc: float) -> dict:
    if clean_auc >= 0.80:
        interpretation = "useful_within_source_evidence_only"
    elif clean_auc >= 0.65:
    ...
    return {
        ...
        "boundary": (
            "A pass is only within-source low-resolution CIFAKE evidence; it is "
            "not proof of broad real-photo or unseen-generator transfer."
        ),
    }
...
def evaluate_candidate(name: str) -> dict:
    ...
    output = model_root / "cifake-matched-source-v12-gate"
    ...
    workshop.atomic_json(progress_path, progress)
    print("MATCHED_GATE_SUMMARY " + json.dumps(progress, sort_keys=True), flush=True)
    return progress
