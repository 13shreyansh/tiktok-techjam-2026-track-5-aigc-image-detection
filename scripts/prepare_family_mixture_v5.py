#!/usr/bin/env python3
"""Build v5 with broad web reals while preserving independent source gates.

V5 changes only the new real branch relative to v4: the first immutable LAION
shard replaces LSUN Church in training.  A byte-disjoint second LAION shard and
the complete Church source remain evaluation-only.  Imagen and PixArt remain
complete fake-generator holdouts.
"""
...
def load(path: Path, label: int | None = None, limit: int | None = None) -> list[dict]:
    rows: list[dict] = []
...
def validate_disjoint(train: list[dict], evaluation: list[dict]) -> dict:
    ...
    if path_overlap:
        raise SystemExit(f"train/evaluation path overlap: {len(path_overlap)}")
    ...
    if hash_overlap:
        raise SystemExit(f"train/evaluation SHA-256 overlap: {len(hash_overlap)}")
    return {
...
def main() -> None:
    ...
    parser.add_argument("--laion-train", type=Path, default=Path("datasets/wildfake_real_laion5b_subset/manifest.jsonl"))
    parser.add_argument("--laion-eval", type=Path, default=Path("datasets/wildfake_real_laion5b_subset_b/manifest.jsonl"))
    parser.add_argument("--laion-eval-count", type=int, default=1024)
    parser.add_argument("--seed", type=int, default=20260829)
    parser.add_argument("--output-root", type=Path, default=Path("datasets/family_mixture_v5"))
    ...
    new_train_reals = load(args.celebahq, label=0) + load(args.laion_train, label=0)
    ...
    church_holdout = load(args.church, label=0)
    laion_holdout = load(args.laion_eval, label=0, limit=args.laion_eval_count)
    ...
    write(train_path, train_rows, args.seed, "v5-train")
    write(selection_path, selection_rows, args.seed, "v5-selection")
    write(content_path, content_rows, args.seed, "v5-content-holdout")
    report = {
        "seed": args.seed,
        "design": "LAION shard A in training; byte-disjoint LAION shard B and complete LSUN Church evaluation-only",
        "forbidden_sources": [
        ...
        "complete_real_holdout": "WildFake-LSUN-Church",
        "disjoint_same_source_real_holdout": {"WildFake-LAION5B": len(laion_holdout)},
